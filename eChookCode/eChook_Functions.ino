...
/**
* @brief Applies the chosen throttle curve to the input percentage
* @return The required motor output as a percentage
*/
float applyThrottleCurve(float normalizedInput) {
  float output = normalizedInput;
  
  switch(CAL_ACTIVE_THROTTLE_CURVE) {
    case CURVE_LINEAR:
    ...
    case CURVE_LOGARITHMIC:
      ...
      // y = 100 * log(1 + x/100) / log(2)
      if (normalizedInput > 0) {
        output = 100.0 * log(1.0 + normalizedInput/100.0) / log(2.0);
      } else {
        output = 0;
      }
      break;
    ...
    case CURVE_S_CURVE:
      // S-curve using cubic approximation
      // Gentle start, aggressive middle, gentle end
      float x = normalizedInput / 100.0;  // Normalize to 0-1
      output = 100.0 * (3*x*x - 2*x*x*x);  // Smoothstep function
      break;
      
    case CURVE_CUSTOM_LUT:
      // Lookup table with linear interpolation
      int index = (int)(normalizedInput / 10.0);
      if (index >= 10) {
        output = CAL_CUSTOM_CURVE_LUT[10];
      } else {
        float lower = CAL_CUSTOM_CURVE_LUT[index];
        float upper = CAL_CUSTOM_CURVE_LUT[index + 1];
        float fraction = (normalizedInput - index * 10.0) / 10.0;
...
float readThrottle() {
  ...
  if (CAL_THROTTLE_VARIABLE) {
    tempThrottle = (tempThrottle / 1023) * referenceVoltage;
    throttleV = tempThrottle;
    
    tempThrottle = constrain(tempThrottle, CAL_THROTTLE_LOW, CAL_THROTTLE_HIGH);

    normalizedInput = ((tempThrottle - CAL_THROTTLE_LOW) / 
                       (float)(CAL_THROTTLE_HIGH - CAL_THROTTLE_LOW)) * 100.0;
  } else {
    throttleV = (tempThrottle / 1023) * referenceVoltage;
    normalizedInput = (tempThrottle > 200) ? 100.0 : 0.0;
  }

  throttleIn = normalizedInput;
  
  // Apply the curve transformation
  float curvedOutput = applyThrottleCurve(normalizedInput);
  
  // Convert back to 0-255 range for PWM
  tempThrottle = curvedOutput * 2.55;

  // Ramping logic
  if (!CAL_THROTTLE_RAMP) {
    currThrtlOut = tempThrottle;  // Ramping disabled - instant response
  } else {
    bool isIncreasing = tempThrottle >= currThrtlOut;
    bool aboveThreshold = tempThrottle > 100;
    
    if (isIncreasing && aboveThreshold) {
      // Ramp up gradually
      if (currThrtlOut < 100) currThrtlOut = 101;  // Skip low range
      currThrtlOut = min(currThrtlOut + 4, (int)tempThrottle);
    } else {
      // Instant response (decreasing or below 40% throttle)
      currThrtlOut = tempThrottle;
    }
  }

  if (CAL_THROTTLE_OUTPUT_EN) {
    analogWrite(MOTOR_OUT_PIN, currThrtlOut);
  } else {
    analogWrite(MOTOR_OUT_PIN, 0);
  }

  return (float)currThrtlOut / 2.55;
}
